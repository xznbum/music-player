    ...
    <fieldset>
      ...
      <div class="audiotracks-edit-row">
        ...
        {% if track.image %}
          ...
          id="delete_image">
          <label id="audiotracks-delete-image-label" for="delete_image">{% trans 'Удалить изображение' %}</label>
        {% endif %}
      </div>
      <div class="audiotracks-edit-row">
      ...
      <p id="save-button">
      <button class="btn btn-primary">{% trans 'Сохранить изменения' %}</button>
      </p>
    </fieldset>
  </form>
</div>
<div class="audiotracks-form" id="delete-form">
  <form action="{% url 'confirm_delete_track' track.id %}">
    <p id="delete-button">
    <button class="btn btn-danger">{% trans 'Удалить этот трек' %}</button>
    </p>
  </form>
</div>
{% endif %}
