...
class Artist(models.Model):
    name = models.CharField("Исполнитель", max_length=200, null=True, blank=True)
...
class Genre(models.Model):
    name = models.CharField("Жанр", max_length=200, null=True, blank=True)
...
class AbstractTrack(models.Model):
    ...
    audio_file = models.FileField("Аудио файл", upload_to=get_audio_upload_path)
    image = ImageWithThumbsField(name="Изображение", upload_to=get_images_upload_path, null=True,
        blank=True, sizes=((48, 48), (200, 200)))
    title = models.CharField("Название", max_length=200, null=True)
    artist = models.ForeignKey(Artist, verbose_name="Исполнитель", related_name="tracks", blank=True, null=True)
    genre = models.ForeignKey(Genre, verbose_name="Жанр", related_name="tracks", blank=True, null=True)
    description = models.TextField("Описание", null=True, blank=True)
    #like = models.IntegerField("Лайки", null=True, blank=True)
    slug = models.SlugField(verbose_name="Slug (окончание URL)")
...
class Playlist(models.Model):
    user = models.ForeignKey(User, verbose_name="Пользователь", related_name="playlists", blank=True, null=True)
...
class Comment(models.Model):
    created_at = models.DateTimeField(auto_now_add=True)
    user = models.ForeignKey(User, verbose_name="Пользователь", related_name="comments", blank=True, null=True)
    track = models.ForeignKey(Track, verbose_name="Трек", related_name="comments", blank=True, null=True)
    text = models.TextField("Текст", null=True, blank=True)
    rating = models.IntegerField(default=0, verbose_name="Оценка", max_length=5)
...
class Like(models.Model):
    user = models.ForeignKey(User, verbose_name="Пользователь", related_name="likes", blank=True, null=True)
    track = models.ForeignKey(Track, verbose_name="Трек", related_name="likes", blank=True, null=True)