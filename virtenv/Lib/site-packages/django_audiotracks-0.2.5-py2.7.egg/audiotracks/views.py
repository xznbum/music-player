...
from audiotracks.models import get_track_model, Comment, Like, Track
from audiotracks.forms import TrackUploadForm, TrackEditForm, CommentForm
...
def track_detail(request, track_slug, username=None):
    ...
    temp = Like.objects.filter(Q(user_id=request.user.id) & Q(track=track))
    if (temp.count() > 0):
        like = temp[0]
    else:
        like = None
    ...
    return render_to_response("audiotracks/detail.html",
                              {'username': username, 'track': track, 'comments': comments, 'formComment': CommentForm(),
...
def top_tracks(request, username=None, page_number=None):
    ...
    #likes.sort()
    #for like in likes:
    #    tracks.append(tracksAndLikes[like])
    tracks.reverse()
    likes.reverse()
    if (len(tracks) > 100):
        tracks = tracks[:100]
